 ...
 <section class="home-section">
	<nav>
		<div class="sidebar-button">
			...
			<span class="dashboard" style="color: #223A68;" ><b>Vendors Management</b></span>
		</div>
		<div class="search-box">
			<button class="btn btn-sm btn-primary" style="color:white; <?php if (strpos($GetId, 'Add') === false) { ?>display:none<?php } ?>" id="addUserBtn">Add Vendor</button>
		</div>
	 </section><br>
...
<script type="text/javascript">
	...
	table = $('#example').DataTable({
		"processing": true,
		"ajax": "<?php echo base_url('GetVendorList')?>", 
		language: {
			paginate: {
 ...
 // for insert
    $(document).ready(function () {
        $("#addUserBtn").click(function () {
            $("#myModal").load("<?php echo base_url("Vendor_Controller/createVendor");?>", function () {
                $("#myModal").modal("show");
            });
        });
    });
...
////////for update////////
		function edit_list(id){
		  ...
		  $("#edit").load("<?php echo base_url("Vendor_Controller/updatedata")?>/"+ id, function () {
                $("#edit").modal("show");
            });
        }
