 ...
 <section class="home-section">
	<nav>
		<div class="sidebar-button">
			...
			<span class="dashboard" style="color: #223A68; " ><b>Funds Allocation</b></span>
		</div>
		<div class="search-box">
			<button class="btn btn-sm btn-primary" style="color:white;  <?php if (strpos($username, 'admin')=== false) { ?>display:none<?php } ?>" id="addFundBtn">Add Funds</button>
		</div>
	 </section><br>
<div class="form">
...
<script type="text/javascript">
	...
	table = $('#example').DataTable({
		"processing": true,
		"ajax": "<?php echo base_url('GetFundList')?>", 
		language: {
			paginate: {
 ...
 // for insert
    $(document).ready(function () {
        $("#addFundBtn").click(function () {
            $("#myModal").load("<?php echo base_url("Funds_Controller/createFund");?>", function () {
                $("#myModal").modal("show");
            });
        });
    });
	...
	//****FOR UPDATE/***
		function edit_list(id){
		  $("#edit").load("<?php echo base_url("Funds_Controller/updatedata")?>/"+ id, function () {
                $("#edit").modal("show");
            });
        }
